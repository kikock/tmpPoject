...
/**
 * @project_name: 后端平台项目
 * @description: 数值单位的实现-计数单位
 * @author: kikock
 * @create_date: 2025-12-15 09:55
 **/
@Component
public class NumericConverter implements UnitConverter<BigDecimal> {
    ...
    public BigDecimal convert(Object value) {
        // 对于数值类型，parse 和 convert 的逻辑是相同的
        return parse(value);
    }
    ...
    public boolean supports(String unit) {
        ...
        return StringUtils.isNotBlank(one) && (unit.split("-")[0].equals(UnitType.INT.name()) || unit.split("-")[0].equals(UnitType.DOUBLE.name()) || unit.split("-")[0].equals(UnitType.NUMERIC.name()));
    }

    @Override
    public UnitType getUnitType() {
        return UnitType.NUMERIC;
    }
}
